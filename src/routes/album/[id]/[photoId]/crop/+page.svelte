<script lang="ts">
	...
	let photoId = $derived(page.params.photoId ?? '');
	let code = $derived(page.url.searchParams.get('code') ?? '');
	let codeQuery = $derived(code ? `?code=${encodeURIComponent(code)}` : '');
	// Back into the album's photo view, preserving any access code.
	let backUrl = $derived(`/album/${albumId}/${photoId}${codeQuery}`);
	...
	onMount(async () => {
		try {
			photo = await photosService.getPhoto(photoId);
		} catch (e) {
	...
	$effect(() => {
		if (!app.loading && !app.isUser) goto(backUrl);
	});
</script>
